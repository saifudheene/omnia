...
"""
Pytest configuration for utils FVT.
...
def pytest_configure(config):
    ...
    markers = {
        ...
        "requires_playbook": "Tests that need to run the playbook (skip in verify mode)",
    }
...
def _apply_dataset_overrides(config):
    ...
    Environment variables (set by run_validation.sh --config mode):
      ...
      OMNIA_SYNC_INPUT_OVERRIDE   — override config["sync_utils_input"]
...
def pytest_sessionstart(session):
    ...
    valid_scenarios = {"log_collector", "negative"}
    module_name = "utils"
