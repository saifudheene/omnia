...
# =============================================================================
# Utils (Log Collector) — Validation Runner
# =============================================================================
# Usage:
...
#
# Options:
#   --suite <name>    Filter by subfolder (collection, metadata, bundle, etc.)
#   --marker <expr>   Filter by pytest marker (sanity, nft, etc.)
#   -v, --verbose     Increase verbosity
#
# FVT Scenarios:
#   log_collector       Full log collection (invoke, metadata, bundle, modes)
...
")"
    ...
    for name in $scenario_names; do
        local run_flag marker_cfg suite_cfg command_cfg dataset_cfg
        eval "$(python3 -c "
import yaml
with open('${CONFIG_FILE}') as f:
    cfg = yaml.safe_load(f) or {}
...
case "$SCENARIO" in
    list|--list)
        ...
        if [ -d "$NFT_DIR" ]; then
            ...
            echo -e "  ${GREEN}nft${NC}  (${nft_count} test files — idempotency)"
        else
    ...
    help|--help|-h|"")
        echo -e "${BLUE}=================================================================${NC}"
        echo -e "${BLUE}  Utils (Log Collector) — Validation Runner${NC}"
        echo -e "${BLUE}=================================================================${NC}"
        echo ""
        echo -e "  End-to-end tests for the ${GREEN}utils${NC} domain (log collector)."
        echo -e "  Runs the collect.yml playbook and verifies log bundles, metadata, and hashes."
        echo -e "  Module: ${CYAN}utils${NC}   Test dir: ${CYAN}test/utils/fvt/${NC}"
        echo ""
        ...
        echo "  --suite <name>    Run only tests in a subfolder (collection, metadata, bundle, modes)"
        ...
        echo "  log_collector    Full log collection tests (invoke, metadata, bundle, modes)"
        echo "  negative         Error handling tests (missing sources, archive failure)"
        echo ""
        echo -e "${YELLOW}MARKERS${NC}"
        echo "  sanity       Baseline must-pass tests"
        echo "  functional   Functional verification tests"
        echo "  nft          Non-functional tests (idempotency)"
        echo ""
        echo -e "${YELLOW}QUICK START${NC}"
        echo ""
        echo "  # Run all log collector tests:"
        echo "  $0 log_collector test"
        echo ""
        echo "  # Verify only (no playbook):"
        echo "  $0 log_collector verify --marker sanity"
        echo ""
        echo "  # Run NFT (idempotency):"
        echo "  $0 nft test"
        echo ""
        ...
        echo "  setup_env.sh         Setup script: --set-password"
        echo ""
        exit 0
        ;;
esac
...
echo -e "${BLUE}=================================================================${NC}"
if [[ "$IS_NFT" == true ]]; then
    echo -e "${BLUE}  Utils — NFT Runner${NC}"
else
    echo -e "${BLUE}  Utils — Validation Runner${NC}"
fi
echo -e "${BLUE}=================================================================${NC}"
