...
"""
Config validation for utils test framework.
...
def _validate_dataset(dataset: str) -> List[str]:
    ...
    (or skips if no REQUIRED_SRC_FILES are defined).
    When *dataset* is set, validates the dataset directory and its
    required files.
    """
    errors: List[str] = []
    if not dataset:
        # Default mode — validate src/ files if any are required
        if not REQUIRED_SRC_FILES:
            return errors
    ...
    for rel_file in REQUIRED_DATASET_FILES:
        if not os.path.isfile(os.path.join(dataset_path, rel_file)):
            errors.append(
                f"Required file missing: datasets/{dataset}/{rel_file}"
            )
    return errors
