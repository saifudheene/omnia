...
# =============================================================================
# Utils (Log Collector) — Test Environment Setup
# =============================================================================
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            echo ""
            echo "Utils (Log Collector) — Test Environment Setup"
            echo ""
            ...
            echo "  test_creds.yml        SSH credentials (encrypted)"
...
_create_and_encrypt_creds() {
    ...
    if [ -f "$CREDS_FILE" ] && [ -z "$_oim_pass" ]; then
        if command -v ansible-vault &>/dev/null && grep -q '^\$ANSIBLE_VAULT' "$CREDS_FILE" 2>/dev/null; then
            ...
            _oim_pass=$(grep -E '^oim_password:' "$_tmp" | sed 's/^oim_password:[[:space:]]*//; s/[\"'\'']//g' || true)
            rm -f "$_tmp"
        else
            _oim_pass=$(grep -E '^oim_password:' "$CREDS_FILE" | sed 's/^oim_password:[[:space:]]*//; s/[\"'\'']//g' || true)
        fi
    fi

    cat > "$CREDS_FILE" << CREDS_EOF
---
# Utils — test credentials
...
if [ -z "$PASSWORD_VALUE" ] && [ "$UPDATE_PASSWORD" = false ] && [ "$SET_PASSWORD" = false ]; then
    if [ -f "$CREDS_FILE" ]; then
        ...
        ok "Re-run with --set-password or --update-password to change"
    else
        warn "No credentials file found (test_creds.yml)"
        warn "For remote mode: bash setup_env.sh --set-password"
    fi
fi

# ─────────────────────────────────────────────────────────────────────────────
# Step 5: Register run_validation + tab completion
# ─────────────────────────────────────────────────────────────────────────────
_inject_tab_completion() {
    local activate_script="$1"
    local marker="# >>> utils-test >>>"
    local marker_end="# <<< utils-test <<<"
    ...
    cat >> "${activate_script}" << UTILS_ACTIVATE_EOF
...
_run_validation_completions() {
    ...
    local special="all nft list help --config --help"
    local options="--suite --marker -v --verbose --debug"
    local markers="sanity functional nft"
    case "\$COMP_CWORD" in
...
case "$INSTALL_MODE" in
    venv)
        ...
        echo "    run_validation log_collector test --marker sanity"
        ;;
    active-venv)
